...
const BusinessTabsWrapper: React.FC = () => {
  ...
  useEffect(() => {
    const fetchComercios = async () => {
      try {
        setLoading(true);
        let response;
        if (activeTab === "cercanos" && typeof window !== "undefined" && navigator.geolocation) {
          await new Promise<void>((resolve) => {
            navigator.geolocation.getCurrentPosition(
              async (pos) => {
                response = await comercioPublicApi.getCercanos(pos.coords.latitude, pos.coords.longitude);
  ...
  return <BusinessTabs comercios={comercios} loading={loading} error={error} activeTab={activeTab} setActiveTab={setActiveTab} />;
};
